...
HRESULT Scene::LoadScene(HWND const g_hWnd)
{
    ...
    hr = CompileShaderFromFile(L"Shaders/Tutorial04.fx", "VS", "vs_4_0", &pVSBlob);
    if (FAILED(hr))
    {
        MessageBox(nullptr,
    ...
    hr = gm->getDevice()->CreateVertexShader(pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), nullptr, &g_pVertexShader);
    if (FAILED(hr))
    {
        pVSBlob->Release();
        return hr;
    ...
    hr = gm->getDevice()->CreateInputLayout(layout, numElements, pVSBlob->GetBufferPointer(),
    ...
    gm->getContext()->IASetInputLayout(g_pVertexLayout);

    // Compile the pixel shader
    ID3DBlob* pPSBlob = nullptr;
    hr = CompileShaderFromFile(L"Shaders/Tutorial04.fx", "PS", "ps_4_0", &pPSBlob);
    if (FAILED(hr))
    {
        MessageBox(nullptr,
            L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
    ...
    hr = gm->getDevice()->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), nullptr, &g_pPixelShader);
    pPSBlob->Release();
    if (FAILED(hr))
        return hr;
...
void Scene::RenderScene()
{
    ...
    // Renders a triangle
    //
    gm->getContext()->VSSetShader(g_pVertexShader, nullptr, 0);
    gm->getContext()->VSSetConstantBuffers(0, 1, &g_pConstantBuffer);
    gm->getContext()->PSSetShader(g_pPixelShader, nullptr, 0);
    gm->getContext()->DrawIndexed(36, 0, 0);        // 36 vertices needed for 12 triangles in a triangle list
